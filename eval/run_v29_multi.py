#!/usr/bin/env python3
"""
v29: Hybrid BM25 + Semantic with weighted RRF, larger context, better prompt.
Changes from v28: 2x semantic weight in RRF, 12K context, chain-of-thought prompt.
"""
import json
import os
import sys
...
class ClawVaultV29(MemorySystem):
    name = "ClawVault-v29"
    ...
    def query(self, question, question_date=None, **kwargs):
        ...
        context = "\n".join(context_parts)
        prompt = f"""You are answering a question about past conversations. The memories below come from MULTIPLE different chat sessions.

CRITICAL INSTRUCTIONS:
- Information needed to answer may be SPREAD ACROSS multiple sessions. Check ALL sessions.
- If the question asks "how many" or "list all", scan EVERY session for relevant items and aggregate.
- If the question asks about a preference or fact, look for the MOST RECENT mention (latest date).
- Think step by step: first identify which sessions contain relevant info, then synthesize.
...
def main():
    ...
    output_file = os.path.join(RESULTS_DIR, "v29-multi-answers.jsonl")

...
This runner keeps the v34 lexical+semantic+RRF pipeline intact and adds a
third structured retrieval signal:
1) ingest-time fact extraction into an in-memory FactStore tuple list,
2) an in-memory EntityGraph with multi-hop BFS traversal,
3) query-time structured retrieval fused with BM25 + semantic via RRF.

Structured enhancements requested:
- rule-based name/age/location/preference/relationship/temporal extraction
- preference-aware boosts
- timestamp-aware temporal ordering

Scoring:
- Reuses v34 scorer when available (default behavior).
- Falls back to the local scorer only when v34 scorer is unavailable.
"""
...
class FactExtractor:
    ...
    Focused on preferences, entities, and temporal facts for v36 retrieval.
    """
    ...
    def __init__(self) -> None:
        ...
        self.identity_patterns = [
            (
                re.compile(
                    r"\b(?:my\s+name\s+is|i\s+am|i'm)\s+(?P<object>[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b"
                ),
                "name",
                0.93,
            ),
        ...
        self.temporal_patterns = [
            (
                re.compile(
                    rf"\b(?:on|at)\s+(?P<when>{date_expr})\s*,?\s*(?P<event>(?:I|We)\s+[^.?!;]{{3,140}})",
                    re.IGNORECASE,
                ),
                0.83,
            ),
            (
                re.compile(
                    rf"\b(?P<event>(?:I|We)\s+[^.?!;]{{3,140}}?)\s+(?:on|at)\s+(?P<when>{date_expr})\b",
                    re.IGNORECASE,
                ),
                0.81,
            ),
    ...
    def extract_facts(
        ...
        *,
        ...
        # Name extraction.
        for pattern, relation, confidence in self.identity_patterns:
            for match in pattern.finditer(text):
                value = self._clean_value(match.group("object"))
                if not value:
                    continue
                key = ("entity", normalize_key(speaker_entity), normalize_relation(relation), normalize_key(value))
                if key in seen:
                    continue
                seen.add(key)
                facts.append(
        ...
        # Temporal extraction from explicit date mentions.
        for pattern, confidence in self.temporal_patterns:
            for match in pattern.finditer(text):
                event = self._clean_value(match.group("event"))
                when = self._clean_value(match.group("when"))
                if not event:
                    continue
                parsed_time = coerce_datetime(when) or base_valid_from
                key = ("temporal", normalize_key(speaker_entity), "happened_on", normalize_key(event))
                if key in seen:
                    continue
                seen.add(key)
                facts.append(
...
class FactStore:
    ...
    def __init__(self, backing_path: str = ".clawvault/facts.jsonl") -> None:
        ...
        self._fact_tuples: List[StructuredFactTuple] = []
    ...
    def fact_tuples(self) -> List[StructuredFactTuple]:
        return list(self._fact_tuples)
...
class ClawVaultV36:
    ...
    - query performs type-adaptive retrieval with structured-first checks for
      preference/entity/temporal queries, then fuses a structured third signal
      with bm25 + semantic lists via RRF.
    """
    ...
    def _structured_preference_hits(self, question: str, at_time: Optional[datetime], limit: int) -> List[RetrievalHit]:
        relation = normalize_relation(infer_preference_relation_from_question(question) or "")
        ...
        boosted: List[Tuple[ExtractedFact, float]] = []
        for fact, base_score in candidates:
            bonus = 0.0
            ...
            if relation and rel == relation:
                bonus += 0.35
            elif relation and relation.startswith("favorite_") and rel.startswith("favorite_"):
                bonus += 0.20
            boosted.append((fact, base_score + bonus))
        boosted.sort(key=lambda pair: pair[1], reverse=True)
        return [self._fact_hit(fact, score, source="fact-preference") for fact, score in boosted[:limit]]
    ...
    def _structured_temporal_hits(self, question: str, at_time: Optional[datetime], limit: int) -> List[RetrievalHit]:
        candidates = self.fact_store.lookup(
            query=question,
            at_time=at_time,
            limit=max(limit * 4, limit + 8),
        )
        qlow = question.lower()
        prefer_oldest = any(token in qlow for token in ("first", "earliest", "initially"))
        prefer_recent = not prefer_oldest or any(token in qlow for token in ("latest", "recent", "last", "currently", "now"))

        if "before" in qlow and at_time is not None:
            candidates = [(fact, score) for fact, score in candidates if fact.valid_from and fact.valid_from <= at_time]
            prefer_recent = True
            prefer_oldest = False
        elif "after" in qlow and at_time is not None:
            candidates = [(fact, score) for fact, score in candidates if fact.valid_from and fact.valid_from >= at_time]
            prefer_recent = False
            prefer_oldest = True

        known = [(fact, score) for fact, score in candidates if fact.valid_from is not None]
        unknown = [(fact, score) for fact, score in candidates if fact.valid_from is None]
        known.sort(key=lambda item: item[0].valid_from, reverse=prefer_recent and not prefer_oldest)
        candidates = [*known, *unknown]

        ranked: List[Tuple[ExtractedFact, float]] = []
        for rank, (fact, base_score) in enumerate(candidates[:limit]):
            # Earlier rank in temporal ordering gets a monotonic bonus.
            ordered_score = base_score + max(0.0, 0.35 - rank * 0.03)
            ranked.append((fact, ordered_score))
        return [self._fact_hit(fact, score, source="fact-temporal") for fact, score in ranked]
    ...
    def query(
        ...
        *,
        ...
        # Merge with RRF: structured is the third signal (with BM25 + semantic).
...
def resolve_input_file(args: argparse.Namespace) -> Path:
    if args.in_file:
        ...
        # Best-effort auto-download for known LongMemEval filenames.
        basename = in_path.name
        url = None
        for fname, candidate_url in LONGMEMEVAL_DATASET_URLS.values():
            if fname == basename:
                url = candidate_url
                break
        if url is None:
        ...
        print(f"Input file missing at {in_path}; downloading from {url}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-local-scorer",
        action="store_true",
        help="Force local fallback scorer instead of v34 scorer.",
    )

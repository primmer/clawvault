...
1) v34-style BM25 + semantic retrieval with LLM answer synthesis,
2) v35 ingest-time fact extraction (entities/preferences/temporal facts),
3) v35 fact store conflict resolution with temporal validity windows,
4) v35 entity graph multi-hop traversal and structured retrieval,
5) RRF fusion across structured + lexical + semantic retrieval streams.

Critical behavior:
- Retrieved context is synthesized by Ollama LLM answer generation.
- v35 improved local heuristic scorer is preserved for evaluation.
"""
...
DEFAULT_OLLAMA_MAX_TOKENS = 128

ANSWER_PROMPT_TEMPLATE = """You are a memory QA assistant.
Use ONLY the context below to answer the question.
If the answer is not in the context, reply exactly: I don't know.
Return a concise answer phrase with no extra explanation.
...
class ClawVaultV36:
    ...
    - query performs type-adaptive retrieval with structured-first checks for
      ...
      lists with RRF.
    - answer performs v34-style LLM synthesis over fused retrieval context.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.ollama_max_tokens = max(16, int(ollama_max_tokens))
        self.ollama_timeout_sec = max(5, int(ollama_timeout_sec))
        self.context_items = max(1, int(context_items))
    ...
    def _format_hit_for_prompt(self, rank: int, hit: RetrievalHit) -> str:
        meta = hit.metadata or {}
        header_parts = [f"source={hit.source}", f"score={hit.score:.4f}"]
        if meta.get("session_id"):
            header_parts.append(f"session={meta['session_id']}")
        if meta.get("turn_index"):
            header_parts.append(f"turn={meta['turn_index']}")
        if meta.get("entity"):
            header_parts.append(f"entity={meta['entity']}")
        if meta.get("relation"):
            header_parts.append(f"relation={meta['relation']}")
        if meta.get("timestamp"):
            header_parts.append(f"time={meta['timestamp']}")

        body = normalize_space(hit.text)
        answer_hint = normalize_space(str(meta.get("answer", "")))
        if answer_hint and answer_hint.lower() not in body.lower():
            body = f"{body}\nanswer_hint: {answer_hint}"
        return f"[{rank}] {', '.join(header_parts)}\n{body}"

    def _build_answer_prompt(self, question: str, result: QueryResult) -> str:
        context_lines: List[str] = []
        for rank, hit in enumerate(result.hits[: self.context_items], start=1):
            context_lines.append(self._format_hit_for_prompt(rank, hit))
        if not context_lines and result.structured_hits:
            for rank, hit in enumerate(result.structured_hits[: self.context_items], start=1):
                context_lines.append(self._format_hit_for_prompt(rank, hit))
    ...
    def ollama_generate(self, prompt: str) -> Optional[str]:
        ...
        try:
            with urlopen(request, timeout=self.ollama_timeout_sec) as response:
                body = response.read().decode("utf-8", errors="replace")
            parsed = json.loads(body)
            generated = self._clean_llm_output(str(parsed.get("response", "")))
            if generated:
                return generated
    ...
    def _fallback_answer(self, question: str, result: QueryResult) -> str:
    ...
    def answer(
        ...
        *,
        ...
        prompt = self._build_answer_prompt(question, result)
        llm_answer = self.ollama_generate(prompt)
        if llm_answer:
            return llm_answer, result
        # Preserve answerability when local Ollama is unavailable.
        return self._fallback_answer(question, result), result
        ...
        predictions.append(
            {
                ...
                "llm_model": adapter.ollama_model,
                "llm_error": adapter.last_llm_error,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Max generated tokens for Ollama answer synthesis.",
    )
    parser.add_argument(
        "--ollama-timeout-sec",
        type=int,
        default=120,
        help="HTTP timeout for Ollama generation calls.",
    )
    parser.add_argument(
        "--context-items",
        type=int,
        default=8,
        help="How many fused hits to include in the LLM prompt context.",
    )

#!/usr/bin/env python3
"""
v38: v37 + LLM-based fact extraction (Gemini Flash) instead of regex patterns.

Key change over v37: Uses Gemini Flash for fact extraction instead of regex patterns.
Hypothesis: LLM extraction will catch more nuanced preferences and facts that regex misses.
v37 regex extraction was a wash vs v34 on same-day evals.
"""
import json
import os
import sys
...
# --- LLM-based Fact Extraction (Gemini Flash) ---
...
FACT_EXTRACTION_PROMPT = """Extract ALL structured facts from this conversation. Return a JSON array of objects:
- type: one of "likes", "dislikes", "favorite", "habit", "identity", "work", "location", "possession", "goal", "allergy", "attribute", "relationship", "event", "decision"
- value: the key fact/preference (concise)
...
def extract_facts_llm(messages, session_idx, date):
    ...
    if not GEMINI_API_KEY:
        print("WARNING: No GEMINI_API_KEY, falling back to regex")
...
def extract_facts(messages, session_idx, date):
    """Primary extraction: LLM with regex fallback."""
...
class ClawVaultV38(MemorySystem):
    name = "ClawVault-v38"
    ...
    def query(self, question, question_date=None, question_type=None, **kwargs):
        ...
        # Only inject facts for preference + knowledge-update (v37 selective injection)
        use_facts = question_type in ("single-session-preference", "knowledge-update")
...
def main():
    ...
    output_file = os.path.join(RESULTS_DIR, "v38-full-answers.jsonl")

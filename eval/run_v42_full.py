#!/usr/bin/env python3
"""
v42: v40 base + LLM fact extraction (Gemini Flash) + facts for ALL question types.

Hypothesis: v40's regex extraction misses most preferences (only catches "I love/like/prefer").
Gemini Flash can extract nuanced preferences ("I tend to...", opinions, implicit prefs).
Also inject facts for ALL types including preference (v40 excluded preference from facts).

v40 baseline: 57.6% Gemini, 61.6% Ollama
Target: 60%+ Gemini (preference category: 40%+)
"""
import json
import os
import sys
...
def main():
    ...
    output_file = os.path.join(RESULTS_DIR, "v42-full-answers.jsonl")

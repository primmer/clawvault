#!/usr/bin/env python3
"""
v43: v42 base + CONDITIONAL fact injection.

v42 showed LLM facts help some categories but HURT others:
  preference: 53.3->70.0 (+16.7pp) HELP | SSU: 52.9->77.1 (+24.2pp) HELP
  knowledge: 57.7->82.1 (+24.4pp) HELP | SSA: 78.6->76.8 (-1.8pp) NEUTRAL
  multi-session: 66.2->60.2 (-6.0pp) HURT | temporal: 58.6->57.1 (-1.5pp) HURT

Strategy: inject facts for preference, SSU, knowledge-update ONLY.
No facts for multi-session, temporal, SSA.

v42 baseline: 67.6% Gemini, 69.6% Ollama
Target: 70%+ Gemini (recover multi-session to 66%+)
"""
import json
import os
import sys
...
class ClawVaultV40(MemorySystem):
    ...
    def query(self, question, question_date=None, question_type=None, **kwargs):
        ...
        # v43: CONDITIONAL facts — only for types where v42 showed gains
        FACT_TYPES = {'single-session-preference', 'single-session-user', 'knowledge-update'}
        if question_type in FACT_TYPES:
            fact_context = self._fact_retrieval(question, top_k=10)
        else:
            fact_context = ''  # no facts for multi-session, temporal, SSA
...
def main():
    ...
    output_file = os.path.join(RESULTS_DIR, "v43-conditional-answers.jsonl")

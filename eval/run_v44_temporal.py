#!/usr/bin/env python3
"""
v44: v42 base + time-aware query expansion for temporal questions.

v42 = 67.6% Gemini / 69.6% Ollama. Temporal is weakest at 57.1%.
This version adds date-range filtering and temporal keyword expansion
to improve temporal-reasoning questions without touching other categories.

v42 baseline: 67.6% Gemini, 69.6% Ollama
Target: 70%+ Gemini (temporal: 65%+)
"""
import json
import os
import sys
...
def extract_facts(messages, session_idx, date):
    """v42: LLM extraction via Gemini Flash — targets preference gap.
    Falls back to regex if Gemini unavailable."""
    return extract_facts_llm(messages, session_idx, date)
...
class ClawVaultV40(MemorySystem):
    name = "ClawVault-v42"
    ...
    def _expand_temporal_query(self, question, question_date):
        """v44: Expand temporal queries with date-aware context.
        
        For temporal-reasoning questions, parse time references and:
        1. Extract date ranges from the question (e.g., 'last month', 'in January')
        2. Add date keywords to boost BM25 matching on date-tagged sentences
        3. Filter/boost sentences within the relevant time window
        """
        import re
        ...
        try:
            ref_date = datetime.strptime(question_date, "%Y-%m-%d")
        ...
        if "last week" in q_lower:
            ...
            end_date = ref_date
        elif "recently" in q_lower or "latest" in q_lower or "most recent" in q_lower:
            start_date = ref_date - timedelta(days=30)
            end_date = ref_date
        elif "first" in q_lower or "earliest" in q_lower or "originally" in q_lower:
            start_date = None  # no filter, but sort ascending
            end_date = ref_date
        elif any(w in q_lower for w in ["before", "prior to", "until"]):
            end_date = ref_date
        elif any(w in q_lower for w in ["after", "since"]):
        ...
        for mname, mnum in months.items():
            if mname in q_lower:
                # Assume same year as question or year before
                year = ref_date.year if mnum <= ref_date.month else ref_date.year - 1
                start_date = datetime(year, mnum, 1)
                if mnum == 12:
                    end_date = datetime(year + 1, 1, 1)
                else:
                    end_date = datetime(year, mnum + 1, 1)
                date_keywords.append(f"{mname} {year}")
                date_keywords.append(f"{year}-{mnum:02d}")
                break
        
        # Parse "how many times" / counting patterns — expand window
        if any(w in q_lower for w in ["how many times", "how often", "how many"]):
            if not start_date:
                start_date = ref_date - timedelta(days=365)
                end_date = ref_date
        
        # Add date strings as query expansion
        expanded = question
        if date_keywords:
            expanded = question + " " + " ".join(date_keywords)
        elif start_date:
            expanded = question + f" {start_date.strftime('%Y-%m')} {start_date.strftime('%B')}"
        
        return expanded, start_date, end_date

    def _temporal_sentence_retrieval(self, question, question_date, top_k=35):
        """v44: Time-aware sentence retrieval for temporal questions.
        
        Like _sentence_retrieval but:
        1. Expands query with temporal keywords
        2. Boosts sentences within the relevant date range
        3. Returns more results (top_k=35) for counting questions
        """
        from datetime import datetime
        
        expanded_q, start_date, end_date = self._expand_temporal_query(question, question_date)
        
        # BM25 with expanded query
        qt = self._tokenize(expanded_q)
        bm25_scores = [(self._bm25_score(qt, i), i) for i in range(len(self.sentences))]
        bm25_scores.sort(reverse=True)
        bm25_top = [(s, i) for s, i in bm25_scores[:50] if s > 0]
        
        # Semantic with original query (expansion can hurt semantic)
        ...
        # Date-range boost: +50% for sentences in the target window
            ...
            for sid in scores:
                ...
                try:
                    ...
                    if end_date and s_date > end_date:
                        in_range = False
                    if in_range:
                        scores[sid] *= 1.5  # 50% boost for date-matched
                except (ValueError, TypeError):
                    pass
        ...
        for sid, _ in fused:
            sess = self.session_of_sent[sid]
            if session_counts[sess] < 7:  # allow more per session for temporal
                selected.append(sid)
                session_counts[sess] += 1
            if len(selected) >= top_k: break
        
        # Sort by date for temporal coherence
        selected.sort(key=lambda sid: self.sentences[sid].get("date", ""))
        ...
        for sid in selected:
            s = self.sentences[sid]
            entry = f"[Session {s['session_idx']}][{s['date']}][{s['role']}] {s['text']}"
            if total + len(entry) > 10000: break  # bigger context for temporal
            parts.append(entry)
            total += len(entry)
        return "\n".join(parts)
    ...
    def query(self, question, question_date=None, question_type=None, **kwargs):
        ...
        if use_session:
            ...
            # v44: use time-aware retrieval for temporal questions
            context = self._temporal_sentence_retrieval(question, question_date, top_k=35)
        else:
            context = self._sentence_retrieval(question, top_k=25)

        # v42: facts for ALL types — LLM extraction should produce better facts
        # that help even preference/SSU (where regex facts hurt due to noise)
        ...
        if question_type == "temporal-reasoning":
            prompt_parts.append("Pay attention to DATES in the memories. The memories are sorted chronologically. Use dates to determine sequence, recency, and frequency.")
...
def main():
    ...
    output_file = os.path.join(RESULTS_DIR, "v44-temporal-answers.jsonl")

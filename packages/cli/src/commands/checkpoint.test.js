...
function listCheckpointHistoryFiles(vaultPath) {
...
describe('checkpoint debounce', () => {
    it('writes each flushed checkpoint to history with timestamped filenames', async () => {
        ...
        try {
            ...
            const latest = JSON.parse(fs.readFileSync(path.join(vaultPath, '.clawvault', 'last-checkpoint.json'), 'utf-8'));
    ...
    it('writes urgent checkpoints immediately and triggers a wake', async () => {
        ...
        try {
            ...
            expect(fs.existsSync(historyDir)).toBe(true);
            expect(execFileSyncMock).toHaveBeenCalledWith('openclaw', expect.arrayContaining(['gateway', 'wake', '--mode', 'now']), expect.objectContaining({ stdio: 'inherit' }));

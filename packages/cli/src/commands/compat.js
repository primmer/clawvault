...
function checkPluginManifest(options) {
    ...
    try {
        ...
        if (!manifest.configSchema)
            issues.push('missing configSchema');
        if (issues.length > 0) {
            return {
                label: 'plugin manifest',
                status: 'error',
...
function checkPluginExtensions(options) {
    ...
    try {
        ...
        const missing = extensions.filter((ext) => !fs.existsSync(path.resolve(baseDir, ext)));
        if (missing.length > 0) {
            return {
                label: 'plugin extensions registration',
                status: 'error',
...
function checkSkillMetadata(options) {
    ...
    try {
        ...
        const metadata = frontmatter.metadata &&
            ...
            : undefined;
        hasOpenClawMetadata = Boolean((metadata &&
...
export function checkOpenClawCompatibility(options = {}) {
    const checks = [
        checkOpenClawCli(),
...
export function compatibilityExitCode(report, options = {}) {
    if (report.errors > 0) {
        return 1;
    }
...
export async function compatCommand(options = {}) {

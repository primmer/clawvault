...
function writeProjectFixture(root) {
    fs.mkdirSync(path.join(root, 'dist', 'plugin'), { recursive: true });
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
        name: 'clawvault',
        openclaw: { extensions: ['./dist/plugin/index.js'] }
    }), 'utf-8');
    fs.writeFileSync(path.join(root, 'openclaw.plugin.json'), JSON.stringify({
        id: 'clawvault',
        kind: 'memory',
        configSchema: { type: 'object', properties: {} }
    }), 'utf-8');
    fs.writeFileSync(path.join(root, 'dist', 'plugin', 'index.js'), 'export default { id: "clawvault", register() {} };', 'utf-8');
...
describe('checkOpenClawCompatibility', () => {
    ...
    it('uses installed package metadata by default when cwd package is unrelated', async () => {
        ...
        try {
            fs.writeFileSync(path.join(cwdFixture, 'package.json'), JSON.stringify({
                name: 'unrelated-app',
                version: '0.0.1'
            }), 'utf-8');
            const { checkOpenClawCompatibility } = await loadCompatModule();
            const report = checkOpenClawCompatibility();
            const extCheck = report.checks.find((check) => check.label === 'plugin extensions registration');
    ...
    it('flags missing plugin manifest', async () => {
        ...
        try {
            ...
            const { checkOpenClawCompatibility } = await loadCompatModule();
            const report = checkOpenClawCompatibility({ baseDir: root });
            const manifestCheck = report.checks.find((check) => check.label === 'plugin manifest');
    ...
    it('flags missing openclaw.extensions in package.json', async () => {
        ...
        try {
            writeProjectFixture(root);
            fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: 'clawvault' }), 'utf-8');
            const { checkOpenClawCompatibility } = await loadCompatModule();
            const report = checkOpenClawCompatibility({ baseDir: root });
            const extCheck = report.checks.find((check) => check.label === 'plugin extensions registration');

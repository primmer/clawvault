...
import { parseObservationLines, readObservations } from '@versatly/clawvault-core/lib/observation-reader.js';
import { estimateTokens, fitWithinBudget } from '@versatly/clawvault-core/lib/token-counter.js';
import { getMemoryGraph } from '@versatly/clawvault-core/lib/memory-graph.js';
import { resolveContextProfile, normalizeContextProfileInput } from '@versatly/clawvault-core/lib/context-profile.js';
...
export function formatContextMarkdown(task, entries) {
...
function getTargetDailyDates(now = new Date()) {
...
function edgeWeight(edge) {
    if (edge.type === 'frontmatter_relation')
        return 0.95;
...
function applyTokenBudget(items, task, budget) {
    ...
    const fitted = fitWithinBudget(items.map((item, index) => ({
...
export async function buildContext(task, options) {
    ...
    const ordered = dedupeContextItems(applySourceCaps(ordering.order.flatMap((group) => grouped[group]), ordering.caps));
...
export async function contextCommand(task, options) {

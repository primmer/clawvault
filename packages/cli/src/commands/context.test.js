...
import { estimateTokens } from '@versatly/clawvault-core/lib/token-counter.js';
const { loadMock, listMock, vsearchMock, readObservationsMock, parseObservationLinesMock, getMemoryGraphMock } = vi.hoisted(() => ({
    loadMock: vi.fn(),
    listMock: vi.fn(),
    vsearchMock: vi.fn(),
...
vi.mock('../lib/observation-reader.js', () => ({
    readObservations: (vaultPath, days) => readObservationsMock(vaultPath, days),
    parseObservationLines: (markdown) => parseObservationLinesMock(markdown)
}));
vi.mock('../lib/memory-graph.js', () => ({
    getMemoryGraph: (vaultPath) => getMemoryGraphMock(vaultPath)
}));

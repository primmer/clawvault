...
import { SessionWatcher } from '@versatly/clawvault-core/observer/watcher.js';
import { observeActiveSessions } from '@versatly/clawvault-core/observer/active-session-observer.js';
import { resolveVaultPath } from '@versatly/clawvault-core/lib/config.js';
import { getObservationPath } from '@versatly/clawvault-core/lib/ledger.js';
const ONE_KIB = 1024;
    ...
    if (options.active || options.cron) {
        ...
        if (options.cron) {
            if (failedSessionCount > 0) {
                ...
                if (firstFailure) {
                    throw new Error(`observer failed for ${failedSessionCount} session(s); first error: ${firstFailure.sessionKey} - ${firstFailure.error}`);
                }
        ...
        if (result.dryRun) {
            console.log(`Dry run: ${result.candidateSessions} session(s) would be observed (${result.totalNewBytes} new bytes).`);
            for (const candidate of result.candidates) {
                console.log(`- ${candidate.sessionKey} [${candidate.sourceLabel}] Δ${candidate.newBytes}B (threshold ${candidate.thresholdBytes}B)`);
            }
            return;
        }
        console.log(`Active observation complete: ${result.observedSessions}/${result.candidateSessions} session(s) observed.${failedSessionCount > 0 ? ` ${failedSessionCount} failed.` : ''}`);
        if (failedSessionCount > 0) {
            for (const failure of result.failedSessions) {
                console.error(`[observer] session failed ${failure.sessionKey} (${failure.sessionId}): ${failure.error}`);
            }
        }
        return;
    }

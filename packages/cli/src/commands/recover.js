...
import { checkDirtyDeath, clearDirtyFlag } from './checkpoint.js';
...
export async function recover(vaultPath, options = {}) {
...
export function formatRecoveryCheckStatus(info) {
    if (!info.died) {
...
export function formatCheckpointList(checkpoints) {
    ...
    const lines = [];
    lines.push(`${headers[0].padEnd(timestampWidth)}  ${headers[1].padEnd(workingOnWidth)}  ${headers[2].padEnd(focusWidth)}  ${headers[3].padEnd(fileWidth)}`);
    lines.push(`${'-'.repeat(timestampWidth)}  ${'-'.repeat(workingOnWidth)}  ${'-'.repeat(focusWidth)}  ${'-'.repeat(fileWidth)}`);
    for (const row of rows) {
        lines.push(`${row.timestamp.padEnd(timestampWidth)}  ${row.workingOn.padEnd(workingOnWidth)}  ${row.focus.padEnd(focusWidth)}  ${row.file}`);
    }
    return lines.join('\n');
}
/**
 ...
 */
export function formatRecoveryInfo(info, options = {}) {

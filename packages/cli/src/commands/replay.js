...
import { normalizeChatGptExport } from '@versatly/clawvault-core/replay/normalizers/chatgpt.js';
import { normalizeClaudeExport } from '@versatly/clawvault-core/replay/normalizers/claude.js';
import { normalizeOpenCodeExport } from '@versatly/clawvault-core/replay/normalizers/opencode.js';
import { normalizeOpenClawTranscript } from '@versatly/clawvault-core/replay/normalizers/openclaw.js';
...
function normalizeReplayMessages(source, inputPath) {
    ...
    if (source === 'opencode') {
        const files = collectFiles(inputPath, (filePath) => filePath.toLowerCase().endsWith('.json') || filePath.toLowerCase().endsWith('.jsonl'));
        if (files.length === 0) {

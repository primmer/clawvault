...
import { getSessionFilePath, getSessionsDir, loadSessionsStore } from '@versatly/clawvault-core/lib/session-utils.js';
...
function resolveAgentId(sessionKey, explicitAgentId) {
    ...
    if (fromEnv)
        return sanitizeAgentId(fromEnv);
    return 'clawdious';
}
...
function resolveSafeTranscriptPath(agentId, sessionId, sessionFile) {
    ...
    for (const candidatePath of candidatePaths) {
        if (path.extname(candidatePath).toLowerCase() !== '.jsonl')
            continue;
...
export function formatSessionRecapMarkdown(result) {
...
export async function buildSessionRecap(sessionKeyInput, options = {}) {
...
export async function sessionRecapCommand(sessionKey, options = {}) {
...
//# sourceMappingURL=session-recap.js.map
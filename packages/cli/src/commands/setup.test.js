...
import { setupCommand, extractPeople, extractPreferences, extractDecisions, extractTasks, extractFromContent, scanAndExtract, importToVault, } from './setup.js';
describe('setup command', () => {
    ...
    it('passes qmd index name when provided', async () => {
        ...
        expect(execFileSyncMock).toHaveBeenCalledWith('qmd', expect.arrayContaining(['--index', 'clawvault-test']), expect.objectContaining({ stdio: 'ignore' }));
    });
});
...
describe('importToVault', () => {
    ...
    it('creates task files in tasks/ directory', () => {
        const extracted = {
            ...
            preferences: [],
            decisions: [],
            tasks: [{ title: 'Write tests', priority: 'high' }],
        };
        const summary = importToVault(vaultDir, extracted, false);

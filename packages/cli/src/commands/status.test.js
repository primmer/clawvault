...
describe('status command', () => {
    ...
    it('returns ok status when everything is healthy', async () => {
        ...
        try {
            ...
            fs.writeFileSync(path.join(clawvaultDir, 'last-checkpoint.json'), JSON.stringify({ timestamp, workingOn: 'sync', focus: null, blocked: null }, null, 2));
            fs.writeFileSync(path.join(clawvaultDir, 'graph-index.json'), JSON.stringify({
                schemaVersion: 1,
                vaultPath,

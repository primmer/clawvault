 ...
 */
import { createTask, listTasks, readTask, updateTask, completeTask, getStatusIcon, getStatusDisplay } from '@versatly/clawvault-core/lib/task-utils.js';
import { queryTransitions, formatTransitionsTable, } from '@versatly/clawvault-core/lib/transition-ledger.js';
/**
 * Add a new task
 */
export function taskAdd(vaultPath, title, options = {}) {
 ...
 */
export function taskList(vaultPath, options = {}) {
    const filters = {};
    if (options.status)
 ...
 * Update a task
 */
export function taskUpdate(vaultPath, slug, options) {
 ...
 * Mark a task as done
 */
export function taskDone(vaultPath, slug, options = {}) {
 ...
 */
export function taskTransitions(vaultPath, taskId, options = {}) {
 ...
 * Show task details
 */
export function taskShow(vaultPath, slug) {
...
export function formatTaskList(tasks) {
    ...
    for (const task of tasks) {
        ...
        if (task.frontmatter.due)
            metaParts.push(`due:${task.frontmatter.due.split('T')[0]}`);
        if (task.frontmatter.tags?.length)
            metaParts.push(task.frontmatter.tags.map(tag => `#${tag}`).join(','));
        if (task.frontmatter.parent)
            metaParts.push(`parent:${task.frontmatter.parent}`);
 ...
 */
export async function taskCommand(vaultPath, action, args) {

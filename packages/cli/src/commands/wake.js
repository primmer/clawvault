...
import { parseObservationMarkdown } from '@versatly/clawvault-core/lib/observation-format.js';
...
export function buildWakeSummary(recovery, recap) {
    let workSummary = '';
...
function readRecentObservationHighlights(vaultPath) {
    ...
    const fileByDate = new Map(listObservationFiles(vaultPath, {
        includeLegacy: true,
...
async function generateExecutiveSummary(recovery, recap, highlights) {
    if (process.env.CLAWVAULT_NO_LLM || process.env.VITEST)
        return null;
...
export async function wake(options) {
    ...
    const highlightSummaryItems = highlights.map((item) => `[${item.type}|i=${item.importance.toFixed(2)}] ${item.text}`);

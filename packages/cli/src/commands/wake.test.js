...
describe('wake', () => {
    it('includes today and yesterday red/yellow observations in recap', async () => {
        ...
        fs.writeFileSync(path.join(vaultPath, 'observations', `${toDateKey(today)}.md`), [
            `## ${toDateKey(today)}`,
            '',
        ...
        fs.writeFileSync(path.join(vaultPath, 'observations', `${toDateKey(yesterday)}.md`), [
            `## ${toDateKey(yesterday)}`,
            '',
    ...
    it('limits wake observation output and appends overflow hint', async () => {
        ...
        fs.writeFileSync(path.join(vaultPath, 'observations', `${todayKey}.md`), [`## ${todayKey}`, '', ...redLines, ...yellowLines].join('\n'), 'utf-8');
        recoverMock.mockResolvedValue({
            died: false,
            checkpoint: null,
            deathTime: null,
...
//# sourceMappingURL=wake.test.js.map
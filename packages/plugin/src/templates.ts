/**
 * Template Registry for ClawVault
 *
 * Parses YAML frontmatter from template files and builds an in-memory
 * registry of {primitiveType → schema}. Supports dynamic observation
 * classification and template-aware file writing.
 */
...
// ============================================================================
// Default Schemas (fallback when templates/ directory is missing)
// ============================================================================
  ...
  for (const line of lines) {
    ...
    // Nested key (field definitions)
      ...
      if (valueStr === '' || valueStr === '|' || valueStr === '>') {
        ...
        // Property of a field
        const fieldObj = nestedObject[nestedKey] as Record<string, unknown>;
        if (fieldObj) {
          if (key === 'enum') {
            fieldObj[key] = [];
          } else {
...
export function initializeTemplateRegistry(templatesDir?: string): TemplateRegistry {
  ...
  // Fall back to defaults if no templates found
...
function convertFrontmatterToSchema(fm: Record<string, unknown>, body: string): TemplateSchema | null {
  const primitive = fm.primitive as string;
  if (!primitive) return null;

  const fields: Record<string, TemplateFieldSchema> = {};
  const fmFields = fm.fields as Record<string, unknown> | undefined;
  ...
  // Extract keywords from primitive name and description
...
function extractKeywordsFromSchema(
  ...
  const keywords: string[] = [primitive];

  // Add primitive name variations
  keywords.push(primitive.replace(/-/g, ' '));
  keywords.push(primitive.replace(/_/g, ' '));

  // Add keywords based on primitive type
  switch (primitive) {
    case 'memory_event':
...
export function classifyText(text: string): ClassificationResult {
  ...
  // Score each primitive based on keyword matches
...
function getKeywordWeight(keyword: string, primitive: string): number {
  // Higher weight for exact primitive name match
  if (keyword === primitive || keyword === primitive.replace(/-/g, ' ')) {
    return 3;
  }

  // Higher weight for strong indicator keywords
...
export function generateFrontmatter(
  ...
  for (const [fieldName, fieldDef] of Object.entries(schema.fields)) {
    // Check if value provided in extraFields
  ...
  // Add source and session if provided

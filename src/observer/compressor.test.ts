...
describe('Compressor', () => {
  ...
  it('filters out raw tool error messages before compression', async () => {
    process.env.ANTHROPIC_API_KEY = '';
    ...
    const output = await compressor.compress(
      [
        'toolresult: {"status": "error", "tool": "exec", "error": "ENOENT"}',
        '{"status": "error", "tool": "read", "error": "File not found"}',
        ...
        'After the deploy failed, we switched to Railway instead of Vercel'
      ],
      ''
    );

    // Tool noise should be filtered out — only the decision-bearing message survives
    expect(output).not.toContain('toolresult');
    expect(output).not.toContain('ENOENT');
    expect(output).not.toContain('npm ERR');
    expect(output).not.toContain('Command exited');
    expect(output).not.toContain('Retry attempt');
    expect(output).not.toContain('ECONNREFUSED');
    expect(output).toContain('switched to Railway');
  });

  it('filters JSON error payloads but keeps non-error JSON', async () => {
    process.env.ANTHROPIC_API_KEY = '';
    process.env.OPENAI_API_KEY = '';
    process.env.GEMINI_API_KEY = '';
    ...
    const output = await compressor.compress(
      [
        ...
        '{"status": "error", "tool": "exec", "error": "timeout"}',
        'Decision: use Redis for caching instead of Memcached'
      ],
      ''
    );
    ...
    expect(output).toContain('Redis for caching');
  });

  it('keeps all messages when none are tool noise', async () => {
    process.env.ANTHROPIC_API_KEY = '';
    process.env.OPENAI_API_KEY = '';
    process.env.GEMINI_API_KEY = '';
    ...
    const output = await compressor.compress(
      [
        ...
        'User preference: always use TypeScript strict mode'
      ],
      ''
    );
